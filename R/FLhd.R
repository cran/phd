...
FLhd <- function(y, X, X1, nperm=2E4, lambda="lambda.min",flip="FALSE", nfolds=10, statistic="partialcor"){
  ...
  if(!(statistic=="partialcor"|statistic=="semipartialcor")){
    stop('argument statistic should be partialcor or semipartialcor ') 
  }
  ...
  X0 = X[,-1]
  Xhatmat  = X0%*%solve( t(X0)%*%X0+lam*diag(q) )%*%t(X0)
  yhat = Xhatmat%*%y      
  ...
  ### TEST BASED ON SEMI-PARTIAL CORRELATION:
  if(statistic=="semipartialcor"){
    ...
    for(j in 2:w){
      ...
      resstar =  sample(resids,size=n,replace=FALSE)
      if(flip==TRUE){
        flips <- 2*rbinom(n,1,0.5)-1
        Ystar <- yhat + flips*resids         
        resstar =  flips*resids 
      }
